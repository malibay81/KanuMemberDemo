...
1. **Repository auf GitHub erstellen**
   ...
   git branch -M main
   git remote add origin https://github.com/DEIN-USERNAME/KanuMemberDemo.git
...
3. **Base-Path** (bereits konfiguriert)
   
   In [`vite.config.ts`](vite.config.ts) ist der `base`-Pfad bereits auf `KanuMemberDemo` gesetzt:
   ```typescript
   base: '/KanuMemberDemo/',
   ```
...
4. **Push und Deployment**
   ...
   ```
   https://DEIN-USERNAME.github.io/KanuMemberDemo/
   ```
